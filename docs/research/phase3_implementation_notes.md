...
2. **Earnings filter via EODHD Upcoming Earnings** — the port uses QC's free `EODHDUpcomingEarnings` dataset (1998–present, daily, ~97% exact-date precision per the QC data catalog). Wiring: `self.add_universe(EODHDUpcomingEarnings, self._on_earnings_data)`. The callback fires daily with every currently-scheduled upcoming report; it updates a side-effect dict `self.earnings_by_symbol[equity_symbol] = report_date` and returns only equity symbols already in our universe so subscriptions don't expand. `_next_earnings_date(symbol)` reads from the dict and ignores stale entries. This is a faithful equivalent of `shared/earnings.py`'s `next_earnings_date`.
   - **Off-by-one correction**: the trader rule (`shared/earnings.py:91-94`) is `days_away > 14 → safe`; the port was originally `< EARNINGS_MIN_DAYS` (rejected 0-13), corrected to `<= EARNINGS_MIN_DAYS` (rejects 0-14 inclusive) in both files.
