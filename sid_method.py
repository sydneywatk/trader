# =============================================================================
# SID Method — QuantConnect / LEAN port  ·  EXPERIMENT HARNESS
# =============================================================================
#
# Built on sid_quantconnect_dynamic.py (faithful dynamic-universe port). Adds a
# parameterized exit policy and date range so the baseline and each tweak run
# from the IDENTICAL compiled code — only a backtest parameter differs. That
# makes every A/B clean (no code drift) and supports a train/test holdout.
#
# PARAMETERS (set per-backtest; all have sensible defaults)
#   exit_mode   : "rsi50" (original take-profit) | "trail" (ATR trailing stop)
#   atr_mult    : trailing-stop distance in ATRs        (default 3.0)
#   max_days    : time-exit cap in trading days         (default 10)
#   start_year  : backtest start year (Jan 1)           (default 2020)
#   end_year / end_month / end_day : backtest end date  (default 2026-04-30)
#
# HOLDOUT PROTOCOL
#   Train = 2020-2023 (start_year=2020, end_year=2023, end_month=12, end_day=31)
#   Test  = 2024-2026 (start_year=2024, end_year=2026, end_month=4,  end_day=30)
#   Tune only on Train. A change "counts" only if it ALSO helps on Test.
#   Judge by expectancy / profit-loss ratio, NOT win rate.
#
# DIAGNOSED FLAW THIS HARNESS ATTACKS
#   Dynamic-universe baseline: 61% WR but net -21.7%, profit-loss ratio 0.60
#   (avg win +0.39% vs avg loss -0.65%). Winners capped by the RSI-50 exit,
#   losers run to a far swing-extreme stop. exit_mode="trail" lets winners run.
# =============================================================================
...
# Sid's actual "Stocks List of Profitable Trades" (from his SID Method deck).
# IMPORTANT: this list is OUTCOME-SELECTED — names where the method profited — so
# it is maximally survivorship/selection-biased by construction. Reproducing a
# high win rate here demonstrates the pipeline executes his rules, NOT predictive
# edge (use universe="dynamic" for the unbiased test). Notably ETF-heavy incl.
# leveraged/inverse ETFs (TQQQ/TNA/TZA/DUST/NUGT) which mean-revert strongly.
# SPY is in his list but handled separately as the benchmark/regime instrument.
SID_WATCHLIST = [
    "AAL", "AAPL", "AMD", "AMGN", "ANF", "AXP", "BA", "BYND", "CAT", "CHPT",
    "CHWY", "COIN", "COST", "CSIQ", "CVS", "CVX", "DFS", "DIA", "DIS", "DRIP",
    "DUST", "ELF", "EXPE", "GDX", "GOLD", "GOOG", "GS", "HD", "HON", "HSBC",
    "HUM", "HUT", "IBM", "INTC", "IWM", "JETS", "JPM", "KBH", "KHC", "KO",
    "LLY", "LULU", "LUV", "LVS", "MA", "MAR", "MCD", "MRK", "MU", "NEM",
    "NKE", "NUE", "NUGT", "PANW", "PBR", "PFE", "PHM", "PLUG", "PYPL", "QQQ",
    "RIOT", "RMBS", "ROK", "ROKU", "RTX", "SLB", "SLV", "SMH", "SQQQ", "TGT",
    "TNA", "TQQQ", "TSLA", "TTWO", "TXN", "TZA", "UNH", "V", "VZ", "WMT",
    "WYNN", "XHB", "XLC", "XLE", "XLF", "XLI", "XLK", "XLP", "XLRE", "XLU",
    "XLV", "XLY",
]

# Rule-based universe (universe="etf_rule"): top-N point-in-time holdings of these
# 15 sector + index ETFs (survivorship-free via QC historical constituents), plus
# the ETFs and leveraged/inverse ETFs themselves as tradeable names.
SECTOR_ETFS = ["XLC", "XLY", "XLP", "XLE", "XLF", "XLV", "XLI", "XLB", "XLRE", "XLK", "XLU"]
INDEX_ETFS = ["SPY", "QQQ", "DIA", "IWM"]
RULE_ETFS = SECTOR_ETFS + INDEX_ETFS
LEVERAGED_ETFS = ["TQQQ", "SQQQ", "TNA", "TZA", "DUST", "NUGT", "DRIP",
                  "GDX", "GDXJ", "SLV", "GOLD", "SMH"]
ETF_TOP_N = 10  # top holdings per ETF by weight
...
# Algorithm
# -----------------------------------------------------------------------------
class SidMethodExperimentAlgorithm(QCAlgorithm):

    def _param(self, name, default, cast):
    ...
    def initialize(self):
        # Parameterized date range + exit policy (defaults reproduce the
        # faithful baseline over the full period).
        start_year = self._param("start_year", 2020, int)
        start_month = self._param("start_month", 1, int)
        start_day = self._param("start_day", 1, int)
        end_year = self._param("end_year", 2026, int)
        end_month = self._param("end_month", 4, int)
        end_day = self._param("end_day", 30, int)
        # tickers: comma-separated override of SID_WATCHLIST (for trade validation).
        self.tickers_override = self._param("tickers", "", str)
        self.exit_mode = self._param("exit_mode", "rsi50", str)
        self.max_days = self._param("max_days", MAX_TRADE_DAYS, int)
        self.atr_mult = self._param("atr_mult", ATR_TRAIL_MULT, float)
        # side: "both" (default) | "long" (OS only) | "short" (OB only)
        self.allowed_side = self._param("side", "both", str)
        # universe: "dynamic" (top-N by $vol, stress test) | "watchlist" (Sid's 99)
        self.universe_mode = self._param("universe", "dynamic", str)
        # use_stop: 1 = resting whole-dollar stop (Sid's rule) | 0 = no hard stop
        self.use_stop = self._param("use_stop", 1, int)
        # earnings_exit: 1 = close ~1 day before earnings | 0 = off.
        # NOTE: Sid's checklist lists ONLY stop-loss + RSI-50 as exits. For a
        # faithful run set max_days=0 (no time exit) and earnings_exit=0.
        self.earnings_exit = self._param("earnings_exit", 1, int)
        # spy_filter / weekly_filter: extra ENTRY gates from the trader repo that
        # are NOT in Sid's published checklist. Default on (prior behavior); set
        # both to 0 for a faithful "checklist-only" run.
        self.use_spy_filter = self._param("spy_filter", 1, int)
        self.use_weekly_filter = self._param("weekly_filter", 1, int)
        ...
        # Split-adjusted prices match TradingView (raw dividend-adjusting shifts
        # RSI/MACD and the whole-number stop levels Sid reads off the chart).
        ...
        if self.universe_mode == "watchlist":
            ...
            if self.tickers_override:
                wl = [t.strip().upper() for t in self.tickers_override.split(",") if t.strip()]
            self._setup_fixed(wl)
        elif self.universe_mode == "etf_rule":
            # Point-in-time holdings of the 15 sector/index ETFs (survivorship-free),
            # plus the ETFs + leveraged/inverse ETFs themselves as tradeable names.
            for etf in RULE_ETFS:
                self.add_universe(self.universe.etf(etf, self.universe_settings, self._etf_constituents))
            self._setup_fixed(RULE_ETFS + LEVERAGED_ETFS)
        else:
        ...
        self.spy = self.add_equity("SPY", Resolution.DAILY,
                                   data_normalization_mode=DataNormalizationMode.SPLIT_ADJUSTED).symbol
        ...
        self.log(f"SID Experiment — universe={self.universe_mode} exit_mode={self.exit_mode} "
                 f"side={self.allowed_side} use_stop={self.use_stop} atr_mult={self.atr_mult} "
                 f"max_days={self.max_days} dates={start_year}-01-01..{end_year}-{end_month:02d}-{end_day:02d}")

    # -------------------------------------------------------------------------
    # Watchlist mode — subscribe Sid's fixed 99 names up front and build their
    # indicators (mirrors the standalone fixed-99 port). Weekly trackers warm
    # from history; price/MACD/ATR auto-warm via automatic_indicator_warm_up.
    # -------------------------------------------------------------------------
    def _etf_constituents(self, constituents):
        ranked = sorted([c for c in constituents if c.weight is not None],
                        key=lambda c: c.weight, reverse=True)
        return [c.symbol for c in ranked[:ETF_TOP_N]]

    def _setup_fixed(self, tickers):
        new_symbols = []
        for ticker in tickers:
            ...
            state.atr = self.atr(symbol, ATR_PERIOD, MovingAverageType.WILDERS, Resolution.DAILY)
    ...
    def on_securities_changed(self, changes):
        ...
        for sec in changes.removed_securities:
            ...
            for ind in (state.rsi, state.macd, state.atr):
                if ind is not None:
                    try:
    ...
    def _entry_conditions_met(self, symbol, state, bar, rsi_now, rsi_prev,
                              spy_long_ok, spy_short_ok):
        # Side gate — allow disabling shorts (or longs) without touching logic.
        if self.allowed_side == "long" and state.pending_signal_type != "OS":
            return False
        if self.allowed_side == "short" and state.pending_signal_type != "OB":
            return False
        ...
        # B. Weekly RSI delta > 3 in trade direction (NOT in Sid's checklist; gated)
    ...
    def _enter_trade(self, symbol, state, bar):
        ...
        state.shares = shares
        state.bars_held = 0
        state.trail_extreme = entry_price
    ...
    # -------------------------------------------------------------------------
    # Exit priority depends on exit_mode:
    #   "rsi50" : (resting stop) -> RSI 50 -> time exit -> earnings   [baseline]
    #   "trail" : (resting stop) -> ATR trailing stop -> time exit -> earnings
    # The resting protective stop_market_order remains in both as the gap floor.
    # -------------------------------------------------------------------------
    def _check_exit(self, symbol, state, bar):
        ...
        if self.exit_mode == "trail":
            if state.atr is not None and state.atr.is_ready:
                atr_val = state.atr.current.value
                if state.trade_side == "Long":
                    state.trail_extreme = max(state.trail_extreme, bar.high)
                    if bar.close <= state.trail_extreme - self.atr_mult * atr_val:
                        self._close_trade(symbol, state, bar.close, "Trailing stop")
                        return
                else:
                    state.trail_extreme = min(state.trail_extreme, bar.low)
                    if bar.close >= state.trail_extreme + self.atr_mult * atr_val:
                        self._close_trade(symbol, state, bar.close, "Trailing stop")
                        return
        else:
        ...
        # Time exit is NOT in Sid's checklist — gate it (max_days=0 disables).
        if self.max_days > 0 and state.bars_held >= self.max_days:
            self._close_trade(symbol, state, bar.close, "Time exit")
            return

        # Pre-earnings exit is NOT in Sid's checklist — gate it (earnings_exit=0 disables).
        if self.earnings_exit and state.next_earnings_date is not None:
    ...
    def on_end_of_algorithm(self):
        ...
        self.log("=" * 60)
        self.log(f"SID Experiment summary — exit_mode={self.exit_mode}")
        self.log("=" * 60)
